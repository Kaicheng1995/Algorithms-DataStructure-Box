...
public class Graph implements Iterable<Integer> {
    ...
    public List<Integer> path(int start, int stop) {
        ...
        for(int i = v.size() - 1; i >= 0; i--) {
            if(v.get(i) == start && isAdjacent(start, stop)) {
                P.add(v.get(i));
                break;
            } else if(isAdjacent(v.get(i), stop)) {
                P.add(v.get(i));
                stop = v.get(i);
            }
        }
    ...
    public ArrayList<Integer> shortestPath(int startV, int endV) {
        ...
        while (fringe.size() != 0) {
            ...
            visited.add(curr);
            for (int i = 0; i < adjLists[curr].size(); i += 1) {
                int neigh = adjLists[curr].get(i).to();
                if (distance.get(neigh) > getEdge(curr, neigh).weight() + distance.get(curr)) {
                    distance.put(neigh, getEdge(curr, neigh).weight() + distance.get(curr));
                    pre.put(neigh, curr);
                }
                if (!visited.contains(neigh) && !fringe.contains(neigh)) {
                    fringe.add(neigh);
                }
            }
        }
        ...
        path.add(endV);
        int pointer = endV;
        while (pointer != startV) {
            pointer = pre.get(pointer);
            path.add(pointer);
        }
