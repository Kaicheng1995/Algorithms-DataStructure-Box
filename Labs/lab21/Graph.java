...
public class Graph {
    ...
    public Graph prims(int start) {
        ...
        for (int i : getAllVertices()) {
            if (!list1.contains(i)) {
                dist.put(i, MinEdge(i, (List) list1));
            }
        }

        int num = getAllVertices().size() - 1;
        while (T.allEdges.size() != num) {
            if (X == null) {
                break;
            }
            currVertex = X.poll();
            Edge newEdge = MinEdge(currVertex, (List) list1);
            if (!list1.contains(currVertex)) {
                list1.add(currVertex);
                T.addVertex(currVertex);
                T.addEdge(newEdge.getSource(), newEdge.getDest(), newEdge.getWeight());
            }
            ...
            for (int i : getNeighbors(currVertex)) {
                if(X.contains(i)){
                    Edge newMinEdge = MinEdge(i, (List) list1);
                    Edge oldMinEdge = dist.get(i);
                    if(newMinEdge.getWeight()<oldMinEdge.getWeight()){
                        dist.replace(i, newMinEdge);
                        X.add(i);
                    }
                }
                if (!T.getAllVertices().contains(i)) {
                    X.add(i);
                }
            }
        }
    ...
    public Edge MinEdge(int c, List<Integer> i) {
        Edge ed = null;
        int min = 100000000;
        if (i == null || i.size() == 0) {
            return null;
    ...
    public Graph kruskals() {
        ...
        UnionFind X = new UnionFind(getAllVertices().size());

        for (Edge e : getAllEdges()) {
            if (!X.connected(e.getSource(), e.getDest())) {
                X.union(e.getSource(), e.getDest());
                T.addEdge(e.getSource(), e.getDest(), e.getWeight());
            }
