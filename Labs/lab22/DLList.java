...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item) {
        // TODO: YOUR CODE HERE
            addLast(item);
            Node tobe = sentinel.prev;
            while (tobe.prev != sentinel && tobe.prev.item.compareTo(tobe.item) > 0) {
    ...
    public DLList<T> mergeSort() {
        ...
        DLList<T> List1 = oneHalf.mergeSort();
        DLList<T> List2 = otherHalf.mergeSort();
    ...
    public DLList<T> quicksort() {
        ...
        list.append(smallElements.quicksort());
        list.addLast(pivot);
        list.append(largeElements.quicksort());
        return list;
    }
